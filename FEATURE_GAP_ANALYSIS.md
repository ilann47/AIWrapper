...
| Traycer | Notification lifecycle, Attention/Recent feed and live-arrival toasts | No | Yes | Yes | `notification-lifecycle.ts`, `notification-occurrence.ts`, `notification-temporal-group.ts`, `notifications-popover.tsx` → `aiwrapper-notifications` | Pure lifecycle modules copied with only type-boundary adaptation; host-bound UI adapted to the authenticated quota/audit feed | wired |
...
- Notifications use Traycer's original lifecycle, occurrence and calendar-grouping algorithms with a persistent per-user quota/audit feed, unread badge, Attention/Recent center and live Sonner toasts.
