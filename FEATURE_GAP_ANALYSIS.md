...
| OpenCodex | Atomic, instance-scoped administrator secret lifecycle | Partial | Yes | Yes | `src/server/management-auth.ts` → `services/codex-wrapper/app/aiwrapper/store.py` | Preserves exclusive temporary creation, fsync, atomic publication and per-runtime scoping for AIWrapper's hashed owner recovery credential | wired and regression-tested |
...
| codex-multi-auth | Aggregate policies, budgets, routing, capability matrix, quota cache and runtime monitor | No | Yes | Yes | `commands/monitor.ts` executed through `governance-bridge.mjs` and rendered by `MultiAuthMonitorPanel.tsx` | Original `runMonitorCommand --json` remains the aggregation authority; FastAPI adds administrator admission and OpenCodex supplies the stat-row/polling UI | wired, read-only |
| codex-profiles | Isolated CODEX_HOME creation, status and doctor diagnostics | Partial | Yes | Yes | Complete package, `profiles.py`, `ProfilesPage.tsx` | FastAPI invokes original `init`, `path` and `doctor --json`; admin UI renders profile, CLI, ChatGPT desktop, isolation-boundary, legacy-clone and workspace-registry fields from the machine-readable contract | wired |
...
| OpenCodex + 9router | Opt-in live topology smoke gate | No | Yes | Yes | `openai-provider-option-runtime-smoke.ts`, `rtk.e2e.test.js` → `scripts/e2e/live-platform-smoke.mjs` | Keeps explicit live authorization, real HTTP boundary, timeout, redaction and cleanup while extending the assertions across AIWrapper authentication, chat, persistence, sharing and ledger | wired and live-verified |
| Traycer + OpenCodex | Real-browser product journey | No | Yes | Yes | Traycer `resolution/helpers.cjs` imported directly; `run-resolution-matrix.cjs` → `scripts/e2e/live-browser-smoke.mjs` | Replaces Electron/zoom assertions with fresh-profile login, onboarding, navigation, SSE chat, history, archive, disconnect, console-error and screenshot checks against the built OpenCodex GUI | wired and Chrome-verified |
| Codex-Wrapper + OpenCodex + 9router | Production same-origin container topology | No | Yes | Yes | Codex-Wrapper Compose/Docker architecture + 9router Nginx/SSE deployment pattern + OpenCodex identity probe → `docker-compose.production.yml`, `infra/gateway/nginx.conf`, production smoke and CI | Keeps upstream runtimes private, publishes one ingress, strips only `/aiwrapper`, requires secrets, preserves streaming, checks dependency readiness and reuses a protected OAuth mount | wired; container execution delegated to Linux CI |
...
- Profiles: creating a user automatically initializes an isolated `CODEX_HOME`; administrators can inspect original codex-profile CLI health, login state, ChatGPT desktop discovery, local-state/account boundary, legacy clone warning, configuration/registry paths and workspace-reference diagnostics without entering filesystem paths manually.
- Multi-account governance: administrators see the original codex-multi-auth aggregate monitor for account policies, budget guards, project routing, capability matrix, quota cache, ledger and runtime without duplicating its calculation logic.
...
- The production gate now crosses the running OpenCodex and Codex-Wrapper processes with a real OAuth-backed SSE request, then proves browser-session admission, model discovery, conversation persistence/update/export, expiring sharing and ledger accounting before cleaning its artifacts.
- A second production gate executes Traycer's original CDP transport against installed Chrome and the built OpenCodex GUI, proving the first-use journey, real streaming chat, history lifecycle, role navigation, logout and a zero-browser-error invariant with screenshot evidence.
...
- Owner recovery keys now follow OpenCodex's instance-scoped atomic secret lifecycle; temporary test databases write beside themselves and can no longer overwrite the configured production credential.
- Historical notification snapshots now seed Traycer's arrival baseline silently; only post-baseline prepends or recurrences create Sonner toasts, preventing old audit events from obscuring a fresh chat.
