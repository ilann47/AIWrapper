...
| Traycer | Notification lifecycle, Attention/Recent feed, live-arrival toasts and stable scrolling | No | Yes | Yes | `notification-lifecycle.ts`, `notification-occurrence.ts`, `notification-temporal-group.ts`, `use-notification-center-arrivals.ts`, `use-notification-center-scroll-anchor.ts`, `notifications-popover.tsx` → `aiwrapper-notifications` | Portable modules copied with only type-boundary adaptation; host-bound UI adapted to the authenticated quota/audit feed | wired |
...
- Notifications use Traycer's original lifecycle, occurrence, live-arrival, scroll-anchor and calendar-grouping algorithms with a persistent per-user quota/audit feed, unread badge, Attention/Recent center, N-new reveal control and live Sonner toasts.
