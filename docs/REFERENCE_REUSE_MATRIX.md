...
| Chat SSE consumer | OpenCodex `src/chat/outbound.ts`, `tests/openai-chat-*`, `tests/sse-*` | `d1f544b` | Ported incremental SSE framing and terminal/error handling to the browser client | `apps/web/src/api.ts` | live browser streaming `CHAT_OK` | integrated adaptation |
| Conversation continuity | Codex App Server generated protocol plus OpenCodex session-affinity patterns | `d1f544b` | Added `thread/read`, persisted session ownership, mandatory `thread/resume` before continuation | `apps/api/src/server.ts`, `packages/db/src/repository.ts`, `packages/codex-runtime/src/turns.ts` | live history, continuation and new-chat tests | integrated adaptation |

## Compatibility decisions

OpenCodex uses Bun APIs, codex-profiles is Bash, and Codex-Wrapper is Python. Most reused work is therefore a traceable port or material adaptation, not a byte-for-byte copy. Runtime-neutral algorithms/contracts were ported; platform-specific calls were replaced with Node APIs. `auth.json` is deliberately not copied into PostgreSQL. See `THIRD_PARTY.md` and `NOTICE` for attribution.
